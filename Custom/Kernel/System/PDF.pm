...
Get Barcode
   ...
   $Barcode = $PDFObject->BarcodeGet(
        Barcode => '123456789',
        Code    => 'ean13',    # (Optional) Default ean13. Other available types are codabar, code128, 2of5int, 3of9
        Zone    => 10,         # (Optional) Default 10. Size of bars
        UMZN    => 25,         # (Optional) Default 25. Upper "mending zone"
        LMZN    => 15,         # (Optional) Default 15. Lower "mending zone"
        Font    => 'Helvetica',# (Optional) Default 'Helvetica.
        FNSZ    => 8,          # (Optional) Default 8. Font size
        X       => 10,         # (Optional) Default 10. Places an XObject on the page in the specified location.
        Y       => 20,         # (Optional) Default 20. Places an XObject on the page in the specified location.
        Scale   => .5,         # (Optional) Default .5 . Size (scaling)
   );

=cut

sub BarcodeWindowSetting {
    my ( $Self, %Param ) = @_;
...
sub BarcodeGet {
    ...
    $BarcodeSetting{-code} = $Param{Barcode};          # message
    $BarcodeSetting{-zone} = $Param{Zone} || 10;       # size of bars
    $BarcodeSetting{-umzn} = $Param{UMZN} || 25;       # upper "mending zone"
    $BarcodeSetting{-lmzn} = $Param{LMZN} || 15;       # lower "mending zone"
  # $BarcodeSetting{-quzn} = $Param{QUZN} || 0;        # quiet zone
  # $BarcodeSetting{-ofwt} = $Param{OFWT} || 0.5;      # overflow width
    $BarcodeSetting{-font}  = $Self->{PDF}->corefont( 'Helvetica' ); # Font
    $BarcodeSetting{-fnsz}  = $Param{FNSZ} || 8;        # font size
  # $BarcodeSetting{-ext}  = 1;                       # extended character set
  # $BarcodeSetting{-extn} = '...';                   # barcode extension
  # $BarcodeSetting{-text} = '';                      # alternative text

    if ($Param{Code} eq 'ean13') {
        $Barcode = $Self->{PDF}->xo_ean13( %BarcodeSetting );
    } elsif ($Param{Code} eq 'codabar') {
        $Barcode = $Self->{PDF}->xo_codabar( %BarcodeSetting );
    } elsif ($Param{Code} eq 'code128') {
        $Barcode = $Self->{PDF}->xo_code128( %BarcodeSetting );
    } elsif ($Param{Code} eq '2of5int') {
        $Barcode = $Self->{PDF}->xo_2of5int( %BarcodeSetting );
    } elsif ($Param{Code} eq '3of9') {
        $Barcode = $Self->{PDF}->xo_3of9( %BarcodeSetting );
    } elsif ($Param{Code} eq 'QR-Code') {

            my $ConfigItemObject = $Kernel::OM->Get('Kernel::System::ITSMConfigItem');
            my $MainObject = $Kernel::OM->Get('Kernel::System::Main');

            my $ID = $ConfigItemObject->ConfigItemLookup(
                ConfigItemNumber => "$Param{Barcode}",
            );
            ...
            my $Link = $HTTPType.'://'.$FQDN.'/otobo/index.pl?Action=AgentITSMConfigItemZoom;ConfigItemID='.$ID;
            ...
            my $img2 = plot_qrcode($Link, \%params);
            $img2->write(file => "/opt/otobo/var/tmp/".$String.".jpg")
                or die "Failed to write Barcode: " . $img2->errstr;

            my $True = $Self->Image(
                File   => "/opt/otobo/var/tmp/".$String.".jpg",  # (jpg|png)
                Type   => 'Reduce',       # (optional) default Reduce (ReturnFalse|Reduce)
...
#    $Gfx->formimage($Barcode, $Param{BarcodePosition_X}, $Param{BarcodePosition_Y}, $Param{Scale});
